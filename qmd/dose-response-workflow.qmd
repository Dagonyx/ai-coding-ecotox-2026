...
This vignette provides a comprehensive, step-by-step workflow for conducting proper dose-response analysis using the `drc` package. We demonstrate the complete analysis process from initial model fitting through model selection, assessment, and reporting.

## Introduction

Dose-response analysis is fundamental in toxicology, ecotoxicology, pharmacology, and related fields. The relationship between dose (or concentration) and biological response often follows non-linear patterns that require specialized statistical methods to characterize accurately.
...
We will use the `ryegrass` dataset, which contains measurements of root length in perennial ryegrass (*Lolium perenne L.*) exposed to different concentrations of ferulic acid, a phenolic compound known to inhibit plant growth.

```{r load-data}
#| fig-alt: "Scatter plot showing ryegrass root length (cm) versus ferulic acid concentration (mM), displaying a decreasing dose-response relationship"
...
```{r basic-plot}
#| fig-alt: "Dose-response curve showing LL.4 model fit to ryegrass data with observed data points and fitted sigmoid curve"
...
```{r qq-plot}
#| fig-alt: "Normal Q-Q plot of residuals from LL.4 model showing points approximately along the diagonal reference line"
...
```{r residual-plot}
#| fig-alt: "Residual plot showing residuals versus fitted values with random scatter around zero horizontal line"
...
#| fig-alt: "Comparison of LL.4 and W1.4 model fits showing two overlapping dose-response curves in blue (LL.4) and red dashed line (W1.4)"
...
When multiple models fit similarly well, model averaging provides a robust approach that accounts for model uncertainty. The `maED()` function computes model-averaged ED estimates using AIC-based weighting.
...
The upper and lower asymptotes (parameters `d` and `c`) can be estimated from the data or fixed based on prior knowledge. Understanding when and how to fix these parameters is crucial for proper dose-response analysis.
...
```{r asymptote-plot}
#| fig-alt: "Comparison of LL.4 and LL.3 models showing effect of asymptote constraints with black solid line (LL.4) and blue dashed line (LL.3)"
...
**Example:**
```{r ll-example}
#| fig-alt: "Log-logistic model (LL.4) fitted to ryegrass data showing typical S-shaped dose-response curve"
...
**Example:**
```{r weibull-example}
#| fig-alt: "Comparison of Weibull Type 1 (blue) and Type 2 (red dashed) models showing asymmetric dose-response curves"
...
Hormesis describes a biphasic dose-response relationship where low doses stimulate a response (increase) while high doses inhibit (decrease). This creates a characteristic inverted U-shape or J-shaped curve.
