...
To render this Rmarkdown simply run:
```{r, eval=FALSE}
rmarkdown::render("./Rmd/standartox-workflow-example.Rmd") # render the Rmd script 
```
...
tmp = file.path("R", "custom_plot_FUN.R")
if(file.exists(tmp)){
  source(tmp) # This will run when sourcing the Rmd in your IDE
} else {
  source(file.path("..",tmp)) # This will run when running rmarkdown::render()
}
```
...
You can inspect the structure of the `stxDb` list object further within your RStudio IDE simply via the `View()` (in R Studio) or via `head()` command:
```{r, eval=FALSE}
#View(stxDb) # Works only in RStudio not Positron
head(stxDb)  # Works in both RStudio and Positron
```
...
Using `View()` allows you to use the search bar in your R Studio IDE.
```{r, eval=FALSE}
View(catalog$endpoint)
```
...
### stx_chem()
If you want to know which chemicals are listed in *standaRtox*, you can use `stx_chem()` to directly access this information. 
```{r, eval=FALSE}
chem.dt <- stx_chem()
View(chem.dt)
```
...
# Inspect the chem.dt_filtered object. How many different chemicals can you find in there?
# hint: View(chem.dt_filtered)
...
# Have a look via `View(sum_stats)`. How would you calculate a summary statistic based on 
...
q1.1 <- stx_query(
  ...
  tax_columns = c("tax_group", "tax_taxon", "tax_genus", 
                  "tax_family","tax_order","tax_class"), # added additional tax columns to my output
...
**Taxonomic Order**:
```{r eval=TRUE, fig.height=7, fig.width=9.5, message=FALSE}
# Main plotting code
q1.1 %>%
  ...
  labs( title = "Endpoints by Chemical and Taxonomic Order",
...
**Taxonomic Class**:
```{r eval=TRUE, fig.height=7, fig.width=9.5}
# Main plotting code
q1.1 %>%
  # 1. Filter for only EC50 and LC50 endpoints
...
out_dir <- file.path("./standartox_query_results") # specify path to out_dir
...
tx_family = taxa.dt$tax_family %>% table %>% sort(decreasing = T) %>% as.data.frame
View(tx_family)

# List all taxonomic orders
tx_order = taxa.dt$tax_order %>% table %>% sort(decreasing = T) %>% as.data.frame
View(tx_order)

# List all taxonomic class
tx_class = taxa.dt$tax_class %>% table %>% sort(decreasing = T) %>% as.data.frame
View(tx_class)
```
...
```{r eval=TRUE, fig.height=10, fig.width=9.5}
...
# convert the concentration column into ug/L. We can do this easily here cause we filtered for "g/l"
# Therefore this is an easy task here. :)
dmag50 = dmag50 %>% mutate(concentration_ug.L = concentration * 10^6)
gmn_dt = dmag50[ , .(gmn = exp(mean(log(concentration_ug.L), na.rm = TRUE))), 
                 .(cas, new_chem_name, tax_genus)]

# plot
dmag50 %>%
  ggplot(.,  aes(y = new_chem_name)) +
  geom_point(aes(x = concentration_ug.L, col = 'All values'), pch = 1, alpha = 0.3) +
  ...
  labs(title = 'LC50 values for Ceriodaphnia & Daphnia',
       subtitle = '50 most tested chemicals', x = 'Concentration [µg/L]') +
  theme_minimal() +
...
**Congratulations! You have successfully completed the StandaRtox workflow example.**

![**Well done!**](../img/congrats.jpg)
